...
def get_credentials(credential) -> str:
    """
    Retrieve the path to the sequencing orders directory from credentials/plasmidsaurus.json.
        
    Returns:
        Dictionary of client_id and client_secret
        
    Raises:
        FileNotFoundError: If plasmidsaurus.json doesn't exist
        KeyError: If 'client_id or client_secret is missing from the json.
    """
    config_file = Path(__file__).parent.parent.parent / "credentials" / "plasmidsaurus.json"
    ...
    if credential not in config:
        raise KeyError(f"{credential} key not found in plasmidsaurus.json")
    
    return config[credential]


def create_seqorder_name(item_code):
    ...
    prefix = 'Plasmidsaurus'
    date = res.json()['done_date'].split('T')[0]
    code = res.json()['code']
    
    seqorder_name = ('_').join([prefix, date, code])
    
    return seqorder_name